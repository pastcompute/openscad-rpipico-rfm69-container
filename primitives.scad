...
module riser_slot_pair(height, gap, wx, wy, tx) {
  ...
  translate([ wx / 2, -gap / 2, 0 ]) rotate(a = 180, v = [ 0, 0, 1 ]) triangularish_riser(height, wx, wy, tx);
}

module baseplate(base_thickness, wx, wy, bendradi = 0) {
  linear_extrude(height = base_thickness, convexity = 2) { baseplate_tmpl(wx, wy, bendradi); }
}
